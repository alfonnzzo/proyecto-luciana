...
export function AppProvider({ children }) {
  const [auth, setAuth] = useState({ isAuthenticated: false, role: null, nombre: null })
  ...
  const logout = () => setAuth({ isAuthenticated: false, role: null, nombre: null })

  const addReport = (report) => {
    setReports((prev) => [
      { ...report, id: Date.now(), estado: 'publicado', fecha: new Date().toISOString().slice(0, 10) },
      ...prev,
    ])
  }
