...
function MunicipalSchedule() {
  ...
  const eventosFiltrados = useMemo(() => {
    const eventos =
      barrioFiltro === 'todos' ? municipalEvents : municipalEvents.filter((e) => e.barrio === barrioFiltro)
  ...
  return (
    <div className="container py-4 py-lg-5">
      <header className="mb-4">
        ...
        <p className="text-muted mb-0">Fumigaciones y descacharreos programados en tu ciudad</p>
      </header>
      ...
      {eventosFiltrados.length === 0 ? (
        <div className="empty-state">
          <i className="bi bi-calendar-x fs-1 text-primary" aria-hidden="true"></i>
          <p className="mb-0">No hay avisos programados para este barrio todavía.</p>
        </div>
      ) : (
...
function EventCard({ evento }) {
  const meta = activityMeta[evento.actividad] ?? { icon: 'bi-calendar-event', className: 'text-bg-secondary' }
    ...
    <div className="card shadow-sm">
      <div className="card-body d-flex gap-3">
        <div
          ...
          style={{ width: '52px', height: '52px', background: 'var(--color-bg-light)' }}
        >
          <i className={`bi ${meta.icon} fs-4 text-primary`} aria-hidden="true"></i>
        </div>
        <div className="flex-grow-1">
          <div className="d-flex flex-wrap gap-2 align-items-center mb-1">
            <span className={`badge ${meta.className}`}>{evento.actividad}</span>
            <span className="fw-semibold">{evento.barrio}</span>
          </div>
          <p className="mb-1 text-capitalize">
            <i className="bi bi-calendar3 me-1 text-muted" aria-hidden="true"></i>
