...
import { diseaseInfo, weatherAlert, notificationSettingsDefault } from '../data/mockData.js'
  ...
  return (
    <div className="container py-4 py-lg-5">
      <header className="mb-4">
        ...
        <p className="text-muted mb-0">Clima, guía de prevención y notificaciones de tu zona</p>
      </header>
        ...
        <div className="d-flex gap-2 flex-wrap mb-3">
          {diseaseInfo.map((disease) => (
            <button
              key={disease.id}
              type="button"
              className={activeDisease === disease.id ? 'btn btn-brand btn-sm' : 'btn btn-brand-outline btn-sm'}
    ...
    <div className="card">
      <div className="card-body">
        <div className="d-flex align-items-center gap-2 mb-3">
          <i className={`bi ${disease.icono} fs-3 text-primary`} aria-hidden="true"></i>
        ...
        <GuideSection title="Síntomas">
          {disease.sintomasIntro && <p className="text-muted small mb-2">{disease.sintomasIntro}</p>}
          {disease.sintomas.map((grupo, index) => (
            <div key={grupo.titulo ?? `grupo-${index}`} className="mb-2">
              {grupo.titulo && <p className="fw-semibold small mb-1">{grupo.titulo}</p>}
...
function GuideSection({ title, children, last }) {
  return (
    <div className={last ? 'mb-0' : 'mb-3 pb-3 border-bottom'} style={{ borderColor: 'var(--color-border)' }}>
              ...
              {diseases.map((disease) => (
                <tr key={disease.id}>
                  <td className="fw-semibold">
                    <i className={`bi ${disease.icono} text-primary me-1`} aria-hidden="true"></i>
                    {disease.nombre}
                  </td>
    ...
    <div className="glass-panel-strong p-4" role="alert">
      <div className="d-flex flex-column flex-sm-row gap-3 align-items-sm-center justify-content-between">
        <div className="d-flex align-items-center gap-3">
          <i className="bi bi-cloud-rain-heavy fs-1 text-primary" aria-hidden="true"></i>
          <div>
            <p className="mb-1 fw-semibold">
              Pronóstico de lluvia a las {alert.horaLluvia}: revisá tu patio
            </p>
            <p className="mb-0 small text-muted">
              {alert.ubicacion} · {alert.temperatura}°C · Humedad {alert.humedad}% · Prob. lluvia{' '}
              {alert.probabilidadLluvia}%
            </p>
          </div>
        </div>
