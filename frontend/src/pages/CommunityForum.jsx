    ...
    <div className="container py-4 py-lg-5">
      <header className="d-flex justify-content-between align-items-start gap-3 mb-4">
        <div>
          ...
          <p className="text-muted mb-0">Historias y experiencias de vecinos de Formosa</p>
        </div>
        <button type="button" className="btn btn-brand flex-shrink-0" onClick={handleCompartirClick}>
      ...
      {posts.length === 0 ? (
        <div className="empty-state">
          ...
          <p className="mb-0">Todavía no hay historias publicadas. ¡Sé el primero en compartir!</p>
        </div>
      ) : (
...
function PostCard({ post, canInteract, canModerate, onToggleLike, onComment, onRemove }) {
  ...
  return (
    <article className="card">
      <div className="card-body">
        ...
        <div className="d-flex align-items-center gap-3 border-top pt-2" style={{ borderColor: 'var(--glass-border)' }}>
          <button
            type="button"
          ...
          >
            <i className={`bi ${post.likedByMe ? 'bi-heart-fill' : 'bi-heart'}`} aria-hidden="true"></i>
            Me gusta {post.likes > 0 && `(${post.likes})`}
          </button>
          <button type="button" className="like-btn" onClick={() => setShowComments((v) => !v)}>
            <i className="bi bi-chat-dots" aria-hidden="true"></i>
            Comentarios {post.comentarios.length > 0 && `(${post.comentarios.length})`}
          </button>
        </div>
          ...
          <div className="mt-3">
            ...
            {canInteract && (
              <form onSubmit={handleCommentSubmit} className="d-flex gap-2 mt-2">
                ...
                />
                <button type="submit" className="btn btn-brand-outline btn-sm flex-shrink-0">
                  Enviar
                </button>
              </form>
            )}
        ...
        <div className="modal-content">
          <form onSubmit={onSubmit}>
            ...
            <div className="modal-body d-flex flex-column gap-3">
              <div>
                <label htmlFor="story-title" className="form-label small fw-semibold">
                  Título
                </label>
                <input
                  id="story-title"
              ...
              <div>
                <label htmlFor="story-tag" className="form-label small fw-semibold">
                  Enfermedad relacionada
                </label>
                <select id="story-tag" className="form-select" value={form.tag} onChange={onChange('tag')}>
              ...
              <div>
                <label htmlFor="story-text" className="form-label small fw-semibold">
                  Tu historia
                </label>
                <textarea
                  id="story-text"
