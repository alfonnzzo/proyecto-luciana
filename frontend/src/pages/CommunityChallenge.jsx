...
const statusMeta = {
  ...
  pendiente: { label: 'Pendiente de validación', className: 'badge-state-pending' },
...
function CommunityChallenge() {
  ...
  const handlePhotoUpload = (taskId, file) => {
    ...
    setTasks((prev) =>
      prev.map((task) =>
        task.id === taskId ? { ...task, estado: 'pendiente', foto: previewUrl } : task,
      ),
    )
  ...
  return (
    <div className="container py-4 py-lg-5">
      <header className="mb-4">
        ...
        <p className="text-muted mb-0">Barrio {neighborhoodChallenge.barrio} · Sumá tareas cívicas y ganá puntos</p>
      </header>
      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content text-center">
          <div className="modal-body py-5">
            <i className="bi bi-trophy display-3 text-primary" aria-hidden="true"></i>
            ...
            <button type="button" className="btn btn-brand mt-2" onClick={onClose}>
              Genial
            </button>
          </div>
        </div>
