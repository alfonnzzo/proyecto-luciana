...
function CommunityChallenge() {
  ...
  const handlePhotoUpload = (taskId, file) => {
    ...
    setTasks((prev) =>
      prev.map((task) =>
        task.id === taskId
          ? { ...task, estado: 'pendiente', foto: previewUrl, motivoRechazo: undefined }
          : task,
      ),
    )
    ...
    <div className="container py-4 py-lg-5">
      <header className="mb-4">
        ...
        <p className="text-muted mb-0">
          Barrio {neighborhoodChallenge.barrio} · {neighborhoodChallenge.vecinosParticipando} de{' '}
          {neighborhoodChallenge.metaVecinos} vecinos participando
        </p>
      </header>

      <section className="card shadow-sm mb-4" aria-labelledby="progress-heading">
        <div className="card-body">
          ...
          <div className="progress" role="progressbar" aria-valuenow={progress} aria-valuemin={0} aria-valuemax={100} style={{ height: '14px' }}>
            <div className="progress-bar" style={{ width: `${progress}%` }} />
          </div>
          <p className="small text-muted mt-2 mb-0">
            Al llegar al 100% del barrio, la Municipalidad habilita un evento o recompensa comunitaria.
          </p>
        </div>
      </section>
      ...
      <section aria-labelledby="tasks-heading">
        ...
        {tasks.length === 0 ? (
          <div className="empty-state">
            <i className="bi bi-clipboard-check fs-1 text-primary" aria-hidden="true"></i>
      ...
      {showGoalModal && <GoalReachedModal onClose={() => setShowGoalModal(false)} />}
    </div>
  )
}
  ...
  return (
    <div className="card shadow-sm">
      <div className="card-body">
        ...
        <label htmlFor={inputId} className="btn btn-outline-primary btn-sm mb-0">
      ...
      <div className="modal-dialog modal-dialog-centered">
        <div className="modal-content text-center">
          <div className="modal-body py-5">
            <i className="bi bi-emoji-laughing display-3 text-primary" aria-hidden="true"></i>
            <h2 id="goal-modal-title" className="h4 fw-bold mt-3">
              ¡Felicitaciones, {neighborhoodChallenge.barrio}!
            </h2>
            <p className="text-muted">
              Tu barrio alcanzó el 100% del desafío. La Municipalidad se pondrá en contacto para coordinar el
              evento/recompensa comunitaria.
            </p>
