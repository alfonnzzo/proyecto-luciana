...
function AdminDashboard() {
  ...
  const resolverSubmission = (id, estado) => {
    setSubmissions((prev) => prev.map((s) => (s.id === id ? { ...s, estado } : s)))
  }
  ...
  const handleEventSubmit = (event) => {
    ...
    setFeedback(`Aviso de ${eventForm.actividad.toLowerCase()} publicado para ${eventForm.barrio}.`)
  ...
  return (
    <div className="container py-4 py-lg-5">
      <header className="mb-4">
        ...
        <p className="text-muted mb-0">{pendientes} tareas esperando validación</p>
      </header>

      <div className="row g-4">
        <section className="col-12 col-lg-7" aria-labelledby="validation-heading">
          ...
          {submissionsFiltradas.length === 0 ? (
            <div className="empty-state">
              <i className="bi bi-check2-circle fs-1 text-primary" aria-hidden="true"></i>
            ...
            <div className="row g-3">
              {submissionsFiltradas.map((submission) => (
                <div className="col-12 col-sm-6" key={submission.id}>
                  <SubmissionCard submission={submission} onResolve={resolverSubmission} />
                </div>
              ))}
            </div>
          )}
        ...
        <section className="col-12 col-lg-5" aria-labelledby="schedule-heading">
          ...
          <div className="card shadow-sm">
            <div className="card-body">
              ...
              <form onSubmit={handleEventSubmit} className="d-flex flex-column gap-3">
                <div>
                  <label htmlFor="event-activity" className="form-label small fw-semibold">
                    Actividad
                  </label>
                  <select
                    id="event-activity"
                ...
                <div>
                  <label htmlFor="event-barrio" className="form-label small fw-semibold">
                    Barrio
                  </label>
                  <select
                    id="event-barrio"
                ...
                <div className="row g-2">
                  <div className="col-6">
                    <label htmlFor="event-date" className="form-label small fw-semibold">
                      Fecha
                    </label>
                    <input
                      id="event-date"
                  ...
                  <div className="col-6">
                    <label htmlFor="event-time" className="form-label small fw-semibold">
                      Franja horaria
                    </label>
                    <input
                      id="event-time"
                ...
                <div>
                  <label htmlFor="event-recs" className="form-label small fw-semibold">
          ...
          {publishedEvents.length > 0 && (
            <div className="mt-3">
              <h3 className="h6 fw-semibold">Avisos publicados en esta sesión</h3>
              <ul className="list-group">
                {publishedEvents.map((evento) => (
                  <li className="list-group-item small" key={evento.id}>
                    <strong>{evento.actividad}</strong> · {evento.barrio} · {evento.fecha} ·{' '}
  ...
  return (
    <div className="card h-100 shadow-sm">
      ...
      <div className="card-body">
        ...
        <p className="small text-muted fst-italic mb-3">{submission.fotoDescripcion}</p>
